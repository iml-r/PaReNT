...
xml = load_relevant_entries('wiktionary_dumps/enwiktionary-20220401-pages-articles-multistream.xml.bz2', language_content=(
            r'==French==\n(.*\n)*.*',
            r'((?<!=)==[A-Z])',
            r'==French==\n(.*\n)*?(----)'
        ))
...
pattern = re.compile(r'{{compound\|fr\|[A-ž|]+}}')
pattern2 = re.compile(r'{{compound\|fr\|.*\}}')
...
for i in pagelist2:
    ...
    rodice.append(match[0].replace("{{compound|fr|", "").replace("}}", ""))
...
# for index, rodic in enumerate(rodice):
#     rodic = rodic.split(" ")
...
df.to_csv("data_raw/French/wiktionary_mined_compounds.tsv", sep="\t")